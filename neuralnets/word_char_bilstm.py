...
class BiLSTM:
    def __init__(self,word_embedding,char_embedding,params=None):
    ...
    def build_model(self):
        ...
        inputNodes = [tokens_input,word_input]
        ...
        word_embedding = Embedding(input_dim=self.params['word2id_size']+2,output_dim=self.params['word_embedding_size'],weights=[self.word_embedding]
                                   ,trainable=False,name='word_embedding')(word_input)

        # word_embedding = Dropout(0.5)(word_embedding)
        word_embedding = TimeDistributed(Dense(self.params['char_embedding_size']))(word_embedding)

        embedding = Add()([char_embedding,word_embedding])
        # embedding = Concatenate(axis=-1)([char_embedding,word_embedding])
        # word_embedding = Dense(self.params['char_embedding_size'])(word_embedding)
        #
        # char_embedding = Add()([char_embedding,word_embedding])
