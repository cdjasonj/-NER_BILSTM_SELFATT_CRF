from neuralnets.radical_word_char_bilstm import BiLSTM
import json
import math
import numpy as np
...
params = {'char2id_size':len(char2id),'char_embedding_size':100,'epochs':25,'earlyStopping':5,'word_embedding_size':300,'word2id_size':len(word2id),
          'rad2id_size':len(Radical2id),'rad_embedding_size':20,'maxlen_rad':5,'rad_hidden_size':64,
          'n_class_labels':len(BIO2id),'save_path':'./models/word_char_lstm.weights'}
...
def pad_radical(data):
    maxlen_rad = params['maxlen_rad']
    for _data in data:
        for idx,radical in enumerate(_data['radicals']):
            if len(radical) < maxlen_rad:
                _data['radicals'][idx] = radical + ['0']*(maxlen_rad - len(radical))
            else:
                _data['radicals'][idx] = radical[:maxlen_rad]
    ...
    for data in batch_data:
        ...
        for _word in data['words']:
            for i in range(len(_word)):
                word.append(word2id.get(_word,1))

        radical = []
        for rad in data['radicals']:
            radical.append([Radical2id.get(_rad, 1) for _rad in rad])

        bio = [BIO2id.get(_bio) for _bio in data['NER_BIO']]
        # bio = np.expand_dims(bio,axis=-1)
        dic['text'] = text
        dic['words'] = word
        dic['radicals'] = radical
        dic['NER_BIO'] = bio

        new_batch_data.append(dic)
    pad_radical(new_batch_data)
...
def trainModel(model):
    for batch in minibatch_iterate_dataset(train_data):
        ...
        inputWords = np.array([data['words'] for data in batch])
        inputRadical = np.array([data['radicals'] for data in batch])

        nnInput.append(inputSentece)
        nnInput.append(inputWords)
        nnInput.append(inputRadical)
...
def predictLabels( model, data):
    ...
    words = [_data['words'] for _data in data]
    radical = [_data['radicals'] for _data in data]
    ...
    for indices in tqdm(sentenceLengths.values()):
        ...
        inputWords = np.array([words[idx] for idx in indices])
        inputRadical = []
        for idx in indices:
            inputRadical.append(radical[idx])
        inputRadical = np.array(inputRadical)
