from neuralnets.word_char_bilstm import BiLSTM
import json
import math
import numpy as np
...
import logging
from tools.load_word2vec import get_char_embedding_matrix,get_word_embedding_matrix
...
word_embedding_matrix = get_word_embedding_matrix(word2id)
char_embedding_matrix = get_char_embedding_matrix(char2id)

params = {'char2id_size':len(char2id),'char_embedding_size':300,'epochs':40,'earlyStopping':10,'word_embedding_size':300,
          'word2id_size':len(word2id),'n_class_labels':len(BIO2id),'save_path':'./models/word_char.weights'}
...
def fit(params,dev_data,test_data):
    ...
    char_lstm = BiLSTM(word_embedding_matrix,char_embedding_matrix,params)
