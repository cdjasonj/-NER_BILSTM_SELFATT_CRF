from neuralnets.char_bilstm import BiLSTM
import json
import math
import numpy as np
...
import logging
from tools.load_word2vec import get_char_embedding_matrix
...
char_embedding = get_char_embedding_matrix(char2id)

params = {'char2id_size':len(char2id),'char_embedding_size':300,'epochs':40,'earlyStopping':10
          ,'n_class_labels':len(BIO2id),'save_path':'./models/char_lstm.weights'}
...
def process_batch_data(batch_data,char2id,word2id,Radical2id,BIO2id):
    ...
    for data in batch_data:
        ...
        word = [word2id.get(_word,1) for _word in data['words']] #1 UNK , 0 pad
        radical = []
...
def trainModel(model):
    for batch in minibatch_iterate_dataset(train_data):
        ...
        Inputs = np.array([data['text'] for data in batch])
...
def predictLabels( model, data):
    ...
    for indices in tqdm(sentenceLengths.values()):
        ...
        #输入数据
        inputData = np.array([sentences[idx] for idx in indices])

        predictions = model.predict(inputData, verbose=False)
...
def fit(params,dev_data,test_data):
    ...
    char_lstm = BiLSTM(char_embedding,params)
