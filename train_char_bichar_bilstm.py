from neuralnets.char_bichar_bilstm import BiLSTM
import json
import math
import numpy as np
...
import logging
from tools.load_word2vec import get_char_embedding_matrix,get_bichar_embedding_matrix
...
params = {'char2id_size':len(char2id),'char_embedding_size':300,'epochs':45,'earlyStopping':5,
          'word_embedding_size':300,'word2id_size':len(word2id),'bichar_embedding_size':300,'bichar2id_size':len(bichar2id)
          ,'n_class_labels':len(BIO2id),'save_path':'./models/char_bichar.weights'}
...
def process_batch_data(batch_data, char2id, word2id, Radical2id, BIO2id):
    ...
    for data in batch_data:
        ...
        # bio = np.expand_dims(bio,axis=-1)char2id_size
...
def fit(params,dev_data,test_data):
    ...
    char_lstm = BiLSTM(char_embedding_matrix,bichar_embedding_matrix,params)
