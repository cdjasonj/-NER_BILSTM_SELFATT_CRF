from neuralnets.word_bichar_char_bilstm import BiLSTM
import json
import math
import numpy as np
...
import logging
from tools.load_word2vec import get_embedding_matrix
...
char_embedding_matrix = get_embedding_matrix(char2id)

params = {'char2id_size':len(char2id),'char_embedding_size':300,'epochs':25,'earlyStopping':5,
          'word_embedding_size':300,'word2id_size':len(word2id),'bichar_embedding_size':100,'bichar2id_size':len(bichar2id)
          ,'n_class_labels':len(BIO2id),'save_path':'./models/word_bichar_char.weights'}
